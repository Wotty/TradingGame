...
public class CandleStickChart extends XYChart<String, Number> {
	...
	public CandleStickChart(CategoryAxis xAxis, NumberAxis yAxis, List<BarData> bars, int maxBarsToDisplay) {
		...
		for (BarData bar : sublist) {
			String label = "" + bar.getBarDay();
			series.getData().add(new XYChart.Data<>(label, bar.getOpen(), bar));
			System.out.println( "Adding bar with date/time: " + bar.getBarDay());
			System.out.println( "Adding bar with price: " + ((int)bar.getOpen()));
		}
	...
	public void addBar(BarData bar) {
		...
		String label = ""+ bar.getBarDay();
		System.out.println( "Adding bar with actual time:  {0}"+bar.getBarDay());
		System.out.println( "Adding bar with formated time: {0}"+ label);

		lastBar = new BarData(bar.getBarDay(), bar.getClose(), bar.getClose(), bar.getClose(), bar.getClose(), 0);
	 ...
	 */
	public void updateLast(int price) {
		if (lastBar != null) {
			lastBar.update(price);
			System.out.println( "Updating last bar with date/time: {0}"+ lastBar.getBarDay());
			...
			System.out.println( "Updating last bar with formatteddate/time: {0}"+ dataSeries.get(0).getData().get(datalength - 1).getXValue());
		}
	}
	...
	protected void layoutPlotChildren() {
		...
		for (int seriesIndex = 0; seriesIndex < getData().size(); seriesIndex++) {
			...
			while (iter.hasNext()) {
				...
				int x = (int) getXAxis().getDisplayPosition(getCurrentDisplayedXValue(item));
				int y = (int) getYAxis().getDisplayPosition(getCurrentDisplayedYValue(item));
				Node itemNode = item.getNode();
				BarData bar = (BarData) item.getExtraValue();
				if (itemNode instanceof Candle && item.getYValue() != null) {
					Candle candle = (Candle) itemNode;

					int close = (int) getYAxis().getDisplayPosition(bar.getClose());
					int high = (int) getYAxis().getDisplayPosition(bar.getHigh());
					int low = (int) getYAxis().getDisplayPosition(bar.getLow());
					int candleWidth = 10;
					// update candle
	...
	private class Candle extends Group {
		...
		public void update(int closeOffset, int highOffset, int lowOffset, int candleWidth) {
			openAboveClose = closeOffset > 0;
			updateStyleClasses();
			highLowLine.setStartY(highOffset);
			highLowLine.setEndY(lowOffset);
			if (candleWidth == -1) {
				candleWidth = (int) bar.prefWidth(-1);
			}
			if (openAboveClose) {
		...
		public void updateTooltip(int open, int close, int high, int low) {
	...
	private class TooltipContent extends GridPane {
		...
		public void update(int open, int close, int high, int low) {
